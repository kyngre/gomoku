...
def jekyll_mode_jit(board):
    ...
    if winning_move[0] != -1 and winning_move[1] != -1:
        return winning_move

    blocking_move = find_winning_move_jit(board, 2)
    if blocking_move[0] != -1 and blocking_move[1] != -1:
        return blocking_move

    four_move = find_pattern_move_jit(board, 1, 4)
    if four_move[0] != -1 and four_move[1] != -1:
        return four_move

    block_four = find_pattern_move_jit(board, 2, 4)
    if block_four[0] != -1 and block_four[1] != -1:
        return block_four

    three_move = find_pattern_move_jit(board, 1, 3)
    if three_move[0] != -1 and three_move[1] != -1:
        return three_move
